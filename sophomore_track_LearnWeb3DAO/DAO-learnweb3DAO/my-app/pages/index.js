...
export default function Home() {
  ...
  const getNumProposalsInDAO = async () => {
    try {
      ...
      // console.log("daoNumProposals: ", daoNumProposals); // return awaiter
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const fetchProposalById = async (id) => {
    try {
      ...
      console.log("fetchProposalById - parsedProposal: ", JSON.stringify(parsedProposal));
  ...
  const fetchAllProposals = async () => {
    try {
      ...
      for (let i = 0; i < numProposals; i++) {
        ...
        console.log("fetchAllProposals - proposal: ", JSON.stringify(proposal));
  ...
  const voteOnProposal = async (proposalId, _vote) => {
    try {
      ...
      console.error(error);
      window.alert(error.t0.error.data.message);
    }
  };
